...
/* called from utils.c */
void destroy_list(Var list) {
  int i;
  Var *pv;

  for (i = list.v.list[0].v.num, pv = list.v.list + 1; i > 0; i--, pv++) free_var(*pv);
...
/* called from utils.c */
Var list_dup(Var list) {
  int i, n = list.v.list[0].v.num;
  Var _new = new_list(n);

  for (i = 1; i <= n; i++) _new.v.list[i] = var_ref(list.v.list[i]);
...
int listforeach(Var list, listfunc func, void *data) { /* does NOT consume `list' */
  int i, n;
  int first = 1;
  int ret;
...
Var setremove(Var list, Var value) {
  int i;
...
Var listset(Var list, Var value, int pos) { /* consumes `list', `value' */
  Var _new = list;
...
static Var doinsert(Var list, Var value, int pos) {
  Var _new;
  int i;
  ...
  _new = new_list(size);
  for (i = 1; i < pos; i++) _new.v.list[i] = var_ref(list.v.list[i]);
  _new.v.list[pos] = value;
  for (i = pos; i <= list.v.list[0].v.num; i++) _new.v.list[i + 1] = var_ref(list.v.list[i]);
...
Var listappend(Var list, Var value) {
  int size = list.v.list[0].v.num + 1;
  return doinsert(list, value, size);
}

Var listdelete(Var list, int pos) {
  Var _new;
  int i;
  int size = list.v.list[0].v.num - 1;
  ...
  _new = new_list(size);
  for (i = 1; i < pos; i++) { _new.v.list[i] = var_ref(list.v.list[i]); }
  for (i = pos + 1; i <= list.v.list[0].v.num; i++) _new.v.list[i - 1] = var_ref(list.v.list[i]);
...
Var listconcat(Var first, Var second) {
  ...
  for (i = 1; i <= lfirst; i++) _new.v.list[i] = var_ref(first.v.list[i]);
  for (i = 1; i <= lsecond; i++) _new.v.list[i + lfirst] = var_ref(second.v.list[i]);
...
Var listrangeset(Var base, int from, int to, Var value) {
  ...
  for (index = 1; index <= lenleft; index++) ans.v.list[++offset] = var_ref(base.v.list[index]);
  for (index = 1; index <= lenmiddle; index++) ans.v.list[++offset] = var_ref(value.v.list[index]);
  for (index = 1; index <= lenright; index++) ans.v.list[++offset] = var_ref(base.v.list[to + index]);
...
Var sublist(Var list, int lower, int upper) {
  if (lower > upper) {
    free_var(list);
    return new_list(0);
  } else {
    ...
    for (i = lower; i <= upper; i++) r.v.list[i - lower + 1] = var_ref(list.v.list[i]);
...
int listequal(Var lhs, Var rhs, int case_matters) {
...
static void stream_add_tostr(Stream *s, Var v) {
  switch (v.type) {
  case TYPE_INT: stream_printf(s, "%" PRIdN, v.v.num); break;
  case TYPE_OBJ: stream_printf(s, "#%" PRIdN, v.v.obj); break;
  case TYPE_STR: stream_add_string(s, v.v.str); break;
  case TYPE_ERR: stream_add_string(s, unparse_error(v.v.err)); break;
  case TYPE_FLOAT: unparse_value(s, v); break;
  case TYPE_MAP: stream_add_string(s, "[map]"); break;
  case TYPE_LIST: stream_add_string(s, "{list}"); break;
  case TYPE_ANON: stream_add_string(s, "*anonymous*"); break;
  case TYPE_WAIF: stream_add_string(s, "[[waif]]"); break;
  case TYPE_BOOL: stream_add_string(s, v.v.truth ? "true" : "false"); break;
  default: panic_moo("STREAM_ADD_TOSTR: Unknown Var type");
  }
}

const char *value2str(Var value) {
...
static int print_map_to_stream(Var key, Var value, void *sptr, int first) {
...
void unparse_value(Stream *s, Var v) {
  switch (v.type) {
  case TYPE_INT: stream_printf(s, "%" PRIdN, v.v.num); break;
  case TYPE_OBJ: stream_printf(s, "#%" PRIdN, v.v.obj); break;
  case TYPE_ERR: stream_add_string(s, error_name(v.v.err)); break;
  case TYPE_FLOAT:
    char buffer[41];
  ...
  case TYPE_STR: {
    ...
    while (*str) {
      switch (*str) {
      case '"':
      case '\\': stream_add_char(s, '\\');
      /* fall thru */
      default: stream_add_char(s, *str++);
      }
    }
    stream_add_char(s, '"');
  } break;
  ...
  case TYPE_ANON: stream_add_string(s, "*anonymous*"); break;
  case TYPE_WAIF: stream_printf(s, "[[class = #%" PRIdN ", owner = #%" PRIdN "]]", v.v.waif->_class, v.v.waif->owner); break;
  case TYPE_BOOL: stream_printf(s, v.v.truth ? "true" : "false"); break;
  default: errlog("UNPARSE_VALUE: Unknown Var type = %d\n", v.type); stream_add_string(s, ">>Unknown value<<");
  }
}
...
int list_sizeof(Var *list) {
  ...
  len = list[0].v.num;
  for (i = 1; i <= len; i++) { size += value_bytes(list[i]); }
...
Var strrangeset(Var base, int from, int to, Var value) {
  /* base and value are free'd */
  int index, offset = 0;
  ...
  for (index = 0; index < lenleft; index++) s[offset++] = base.v.str[index];
  for (index = 0; index < lenmiddle; index++) s[offset++] = value.v.str[index];
  for (index = 0; index < lenright; index++) s[offset++] = base.v.str[index + to];
  s[offset] = '\0';
  ans.v.str = s;
  free_var(base);
  free_var(value);
...
Var substr(Var str, int lower, int upper) {
  Var r;
  ...
  if (lower > upper)
    ...
    for (loop = lower - 1; loop < upper; loop++) s[index++] = str.v.str[loop];
    s[index] = '\0';
    r.v.str = s;
  }
  free_var(str);
...
static package make_space_pack() {
...
static package bf_length(Var arglist, Byte next, void *vdata, Objid progr) {
  Var r;
...
static package bf_setadd(Var arglist, Byte next, void *vdata, Objid progr) {
  Var r;
...
static package bf_setremove(Var arglist, Byte next, void *vdata, Objid progr) {
  Var r;
...
static package bf_listappend(Var arglist, Byte next, void *vdata, Objid progr) {
  return insert_or_append(arglist, 1);
}


static package bf_listinsert(Var arglist, Byte next, void *vdata, Objid progr) {
  return insert_or_append(arglist, 0);
}


static package bf_listdelete(Var arglist, Byte next, void *vdata, Objid progr) {
  Var r;
  if (arglist.v.list[2].v.num <= 0 || arglist.v.list[2].v.num > arglist.v.list[1].v.list[0].v.num) {
    free_var(arglist);
...
static package bf_listset(Var arglist, Byte next, void *vdata, Objid progr) {
  Var r;
...
static package bf_equal(Var arglist, Byte next, void *vdata, Objid progr) {
  Var r;
...
static package bf_explode(Var arglist, Byte next, void *vdata, Objid progr) {
  ...
  if (adjacent_delim) {
    while ((found = strsep(&return_string, delim)) != nullptr) ret = listappend(ret, str_dup_to_var(found));
  } else {
...
static package bf_reverse(Var arglist, Byte next, void *vdata, Objid progr) {
  Var ret;
  ...
  if (arglist.v.list[1].type == TYPE_LIST) {
    ...
    for (size_t x = elements, y = 1; x >= 1; x--, y++) { ret.v.list[y] = var_ref(arglist.v.list[1].v.list[x]); }
    ...
    if (len <= 1) {
      ...
      for (size_t x = 0, y = len - 1; x < len; x++, y--) new_str[x] = arglist.v.list[1].v.str[y];
      new_str[len] = '\0';
      ret.type = TYPE_STR;
      ret.v.str = new_str;
    }
...
static package bf_slice(Var arglist, Byte next, void *vdata, Objid progr) {
  ...
  for (int x = 1; x <= alist.v.list[0].v.num; x++) {
    ...
    if (
      (element.type != TYPE_LIST && element.type != TYPE_STR && element.type != TYPE_MAP) ||
    ...
    if (index.type == TYPE_STR) {
      ...
      for (int y = 1; y <= index.v.list[0].v.num; y++) {
        if (index.v.list[y].v.num > (element.type == TYPE_STR ? memo_strlen(element.v.str) : element.v.list[0].v.num)) {
          ...
          tmp = listappend(
            tmp, (element.type == TYPE_STR ? substr(var_ref(element), index.v.list[y].v.num, index.v.list[y].v.num) :
                                             var_ref(element.v.list[index.v.list[y].v.num])));
        }
      }
...
void sort_callback(const Var arglist, Var *ret) {
  ...
  struct VarCompare {
    ...
    bool operator()(const size_t a, const size_t b) const {
      ...
      switch (rhs.type) {
      case TYPE_INT: return lhs.v.num < rhs.v.num;
      case TYPE_FLOAT: return lhs.v.fnum < rhs.v.fnum;
      case TYPE_OBJ: return lhs.v.obj < rhs.v.obj;
      case TYPE_ERR: return ((int)lhs.v.err) < ((int)rhs.v.err);
      case TYPE_STR: return (m_Natural ? strnatcasecmp(lhs.v.str, rhs.v.str) : strcasecmp(lhs.v.str, rhs.v.str)) < 0;
      default: errlog("Unknown type in sort compare: %d\n", rhs.type); return 0;
      }
    }
    const Var *m_Arglist;
  ...
  *ret = new_list(s.size());
  ...
  int moo_list_pos = 0;
  for (const auto &it : s) { ret->v.list[++moo_list_pos] = var_ref(arglist.v.list[1].v.list[it]); }
}
...
static package bf_all_members(Var arglist, Byte next, void *vdata, Objid progr) {
  char *human_string = nullptr;
...
static package bf_strsub(Var arglist, Byte next, void *vdata, Objid progr) { /* (source, what, with [, case-matters]) */
  int case_matters = 0;
  Stream *s;
  package p;
  ...
  try {
    Var r;
    stream_add_strsub(s, arglist.v.list[1].v.str, arglist.v.list[2].v.str, arglist.v.list[3].v.str, case_matters);
    r.type = TYPE_STR;
...
static package bf_strcmp(Var arglist, Byte next, void *vdata, Objid progr) { /* (string1, string2) */
  Var r;
...
static package bf_strtr(Var arglist, Byte next, void *vdata, Objid progr) { /* (subject, from, to [, case_matters]) */
  Var r;
  int case_matters = 0;
  ...
  if (arglist.v.list[0].v.num > 3)
    case_matters = is_true(arglist.v.list[4]);
  r.type = TYPE_STR;
  r.v.str = str_dup(strtr(
    arglist.v.list[1].v.str, memo_strlen(arglist.v.list[1].v.str), arglist.v.list[2].v.str, memo_strlen(arglist.v.list[2].v.str), arglist.v.list[3].v.str,
    memo_strlen(arglist.v.list[3].v.str), case_matters));
  free_var(arglist);
  return make_var_pack(r);
}

static package bf_index(Var arglist, Byte next, void *vdata, Objid progr) { /* (source, what [, case-matters [, offset]]) */
  Var r;
  int case_matters = 0;
  int offset = 0;
  ...
  r.type = TYPE_INT;
  r.v.num = strindex(
    arglist.v.list[1].v.str + offset, memo_strlen(arglist.v.list[1].v.str) - offset, arglist.v.list[2].v.str, memo_strlen(arglist.v.list[2].v.str),
    case_matters);
...
static package bf_rindex(Var arglist, Byte next, void *vdata, Objid progr) { /* (source, what [, case-matters [, offset]]) */
  Var r;
  ...
  r.type = TYPE_INT;
  r.v.num = strrindex(
    arglist.v.list[1].v.str, memo_strlen(arglist.v.list[1].v.str) + offset, arglist.v.list[2].v.str, memo_strlen(arglist.v.list[2].v.str), case_matters);

  free_var(arglist);
  return make_var_pack(r);
}

static package bf_tostr(Var arglist, Byte next, void *vdata, Objid progr) {
  package p;
  ...
  try {
    ...
    for (i = 1; i <= arglist.v.list[0].v.num; i++) { stream_add_tostr(s, arglist.v.list[i]); }
    r.type = TYPE_STR;
    r.v.str = str_dup(stream_contents(s));
    p = make_var_pack(r);
  }
...
static package bf_toliteral(Var arglist, Byte next, void *vdata, Objid progr) {
  package p;
  Stream *s = new_stream(100);
...
static void setup_pattern_cache() {
  int i;
  ...
  for (i = 0; i < PATTERN_CACHE_SIZE - 1; i++) pat_cache_entries[i].next = &(pat_cache_entries[i + 1]);
  pat_cache_entries[PATTERN_CACHE_SIZE - 1].next = nullptr;

  pat_cache = &(pat_cache_entries[0]);
}

static Pattern get_pattern(const char *string, int case_matters) {
  ...
  while (1) {
    if (entry->string && !strcmp(string, entry->string) && case_matters == entry->case_matters) {
...
Var do_match(Var arglist, int reverse) {
  ...
  pat = get_pattern(pattern, (arglist.v.list[0].v.num == 3 && is_true(arglist.v.list[3])));
  ...
  if (!pat.ptr) {
    ...
    default: panic_moo("do_match:  match_pattern returned unfortunate value.\n");
    /*notreached*/
    case MATCH_SUCCEEDED:
      ...
      break;
    case MATCH_FAILED: ans = new_list(0); break;
    case MATCH_ABORTED:
      ans.type = TYPE_ERR;
      ans.v.err = E_QUOTA;
      break;
...
static package bf_match(Var arglist, Byte next, void *vdata, Objid progr) {
  Var ans;
...
static package bf_rmatch(Var arglist, Byte next, void *vdata, Objid progr) {
  Var ans;
...
int invalid_pair(int num1, int num2, int max) {
  if ((num1 == 0 && num2 == -1) || (num1 > 0 && num2 >= num1 - 1 && num2 <= max))
    return 0;
  else
    return 1;
}

int check_subs_list(Var subs) {
  const char *subj;
  int subj_length, loop;

  if (
    subs.type != TYPE_LIST || subs.v.list[0].v.num != 4 || subs.v.list[1].type != TYPE_INT || subs.v.list[2].type != TYPE_INT ||
    subs.v.list[3].type != TYPE_LIST || subs.v.list[3].v.list[0].v.num != 9 || subs.v.list[4].type != TYPE_STR)
    return 1;
  subj = subs.v.list[4].v.str;
  subj_length = memo_strlen(subj);
  if (invalid_pair(subs.v.list[1].v.num, subs.v.list[2].v.num, subj_length))
    return 1;
  ...
  for (loop = 1; loop <= 9; loop++) {
    ...
    if (
      pair.type != TYPE_LIST || pair.v.list[0].v.num != 2 || pair.v.list[1].type != TYPE_INT || pair.v.list[2].type != TYPE_INT ||
      invalid_pair(pair.v.list[1].v.num, pair.v.list[2].v.num, subj_length))
      return 1;
  }
  return 0;
}

static package bf_substitute(Var arglist, Byte next, void *vdata, Objid progr) {
  int template_length;
  ...
  try {
    while ((c = *(_template++)) != '\0') {
      if (c != '%')
        ...
        while (start <= end) stream_add_char(s, subject[start++]);
      }
    }
    ans.type = TYPE_STR;
...
static package bf_value_bytes(Var arglist, Byte next, void *vdata, Objid progr) {
  Var r;
...
static package bf_decode_binary(Var arglist, Byte next, void *vdata, Objid progr) {
  int length;
...
static int encode_binary(Stream *s, Var v, int minimum, int maximum) {
  int i;
...
static package bf_encode_binary(Var arglist, Byte next, void *vdata, Objid progr) {
  Var r;
  package p;
  Stream *s = new_stream(100);
  ...
  try {
    if (encode_binary(s, arglist, 0, 255)) {
      stream_add_raw_bytes_to_binary(s2, stream_contents(s), stream_length(s));
      r.type = TYPE_STR;
...
static package bf_parse_ansi(Var arglist, Byte next, void *vdata, Objid progr) {
...
static package bf_remove_ansi(Var arglist, Byte next, void *vdata, Objid progr) {
