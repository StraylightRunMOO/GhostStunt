...
static malloc_fn __malloc = NULL;
static realloc_fn __realloc = NULL;
static free_fn __free = NULL;
...
void hashmap_set_allocator(malloc_fn malloc, free_fn free) {
    __malloc = malloc;
    __free = free;
}
...
static void *malloc_default(size_t size, bool map) {
    return malloc(size);
}

static void *realloc_default(void *ptr, size_t size, bool map) {
    return realloc(ptr, size);
}

static void free_default(void *ptr, bool map) {
    free(ptr);
}

// hashmap_new_with_allocator returns a new hash map using a custom allocator.
// See hashmap_new for more information information
struct hashmap *hashmap_new_with_allocator(malloc_fn _malloc, 
    realloc_fn _realloc, free_fn _free,
    ...
    void *udata)
{
    _malloc = _malloc ? _malloc : __malloc ? __malloc : malloc_default;
    _realloc = _realloc ? _realloc : __realloc ? __realloc : realloc_default;
    _free = _free ? _free : __free ? __free : free_default;
    size_t ncap = 16;
    if (cap < ncap) {
        cap = ncap;
    } else {
    ...
    struct hashmap *map = _malloc(size, true);
    if (!map) {
        return NULL;
    }
    ...
    map->buckets = _malloc(map->bucketsz*map->nbuckets, false);
    if (!map->buckets) {
        _free(map, true);
        return NULL;
    }
...
void hashmap_clear(struct hashmap *map, bool update_cap) {
    ...
    if (update_cap) {
        ...
        void *new_buckets = map->malloc(map->bucketsz*map->cap, false);
        if (new_buckets) {
            map->free(map->buckets, false);
...
size_t hashmap_nbuckets(struct hashmap *map) {
    return map->nbuckets;
}
...
bool hashmap_iter(struct hashmap *map, size_t *i, void **item, bool reverse) {
    struct bucket *bucket;

    if(reverse && *i >= map->nbuckets)
        *i = map->nbuckets - 1;
    else if(!reverse && *i == 0)
        *i = 1;

    do {
        if (*i > map->nbuckets || *i == 0) return false;
        bucket = bucket_at(map, *i - 1);
        reverse ? (*i)-- : (*i)++;
    } while (!bucket->dib);
