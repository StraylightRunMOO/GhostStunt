...
int parse_number(const char *str, Num *result, int try_floating_point) {
  char *p;

  *result = (Num)strtoimax(str, &p, 10);
  if (try_floating_point && (p == str || *p == '.' || *p == 'e' || *p == 'E'))
...
static int parse_object(const char *str, Objid *result) {
  Num number;
...
int parse_float(const char *str, double *result) {
  char *p;
  int negative = 0;
...
enum error become_integer(Var in, Num *ret, int called_from_toint) {
  switch (in.type) {
  case TYPE_INT: *ret = in.v.num; break;
  case TYPE_STR:
    if (!(called_from_toint ? parse_number(in.v.str, ret, 1) : parse_object(in.v.str, ret)))
      *ret = 0;
    break;
  case TYPE_OBJ: *ret = in.v.obj; break;
  case TYPE_ERR: *ret = in.v.err; break;
  case TYPE_FLOAT:
    if (!IS_REAL(in.v.fnum))
      return E_FLOAT;
    *ret = (Num)in.v.fnum;
    break;
  case TYPE_BOOL: *ret = in.v.truth; break;
  case TYPE_MAP:
  case TYPE_LIST:
  case TYPE_ANON:
  case TYPE_WAIF: return E_TYPE;
  default: errlog("BECOME_INTEGER: Impossible var type: %d\n", (int)in.type);
  }
  return E_NONE;
}

static enum error become_float(Var in, double *ret) {
  switch (in.type) {
  case TYPE_INT: *ret = (double)in.v.num; break;
  case TYPE_STR:
    if (!parse_float(in.v.str, ret) || !IS_REAL(*ret))
      return E_INVARG;
    break;
  case TYPE_OBJ: *ret = (double)in.v.obj; break;
  case TYPE_ERR: *ret = (double)in.v.err; break;
  case TYPE_FLOAT: *ret = in.v.fnum; break;
  case TYPE_MAP:
  case TYPE_LIST:
  case TYPE_ANON:
  case TYPE_WAIF:
  case TYPE_BOOL: return E_TYPE;
  default: errlog("BECOME_FLOAT: Impossible var type: %d\n", (int)in.type);
  }
  return E_NONE;
}
...
int do_equals(Var lhs, Var rhs) { /* LHS == RHS */
...
int compare_integers(Num a, Num b) {
  if (a < b)
    return -1;
  else if (a > b)
    return 1;
...
Var do_power(Var lhs, Var rhs) { /* LHS ^ RHS */
  Var ans;
  ...
  if (lhs.type == TYPE_INT) { /* integer exponentiation */
    ...
    if (b < 0) {
      switch (a) {
      case -1: ans.v.num = (b & 1) ? 1 : -1; break;
      case 0:
        ans.type = TYPE_ERR;
        ans.v.err = E_DIV;
        break;
      case 1: ans.v.num = 1; break;
      default: ans.v.num = 0; break;
      }
    } else {
      r = 1;
      while (b != 0) {
...
static package bf_toint(Var arglist, Byte next, void *vdata, Objid progr) {
  Var r;
  enum error e;
...
static package bf_tofloat(Var arglist, Byte next, void *vdata, Objid progr) {
  Var r;
  enum error e;
...
static package bf_min(Var arglist, Byte next, void *vdata, Objid progr) {
  Var r;
...
static package bf_max(Var arglist, Byte next, void *vdata, Objid progr) {
  Var r;
  int i, nargs = arglist.v.list[0].v.num;
  int bad_types = 0;
...
static package bf_abs(Var arglist, Byte next, void *vdata, Objid progr) {
  Var r;
...
#define MATH_FUNC(name)                                                                                                                                       \
  static package bf_##name(Var arglist, Byte next, void *vdata, Objid progr) {                                                                                \
    errno = 0;                                                                                                                                                \
...
static package bf_trunc(Var arglist, Byte next, void *vdata, Objid progr) {
  double d;
...
static package bf_atan(Var arglist, Byte next, void *vdata, Objid progr) {
  double d, dd;
...
static package bf_atan2(Var arglist, Byte next, void *vdata, Objid progr) {
...
static package bf_time(Var arglist, Byte next, void *vdata, Objid progr) {
  Var r;
  r.type = TYPE_INT;
...
static package bf_ctime(Var arglist, Byte next, void *vdata, Objid progr) {
...
static package bf_ftime(Var arglist, Byte next, void *vdata, Objid progr) {
#ifdef __MACH__
...
static package bf_random(Var arglist, Byte next, void *vdata, Objid progr) {
...
static package bf_reseed_random(Var arglist, Byte next, void *vdata, Objid progr) {
  free_var(arglist);
...
static package bf_frandom(Var arglist, Byte next, void *vdata, Objid progr) {
...
static package bf_round(Var arglist, Byte next, void *vdata, Objid progr) {
...
static package bf_random_bytes(Var arglist, Byte next, void *vdata, Objid progr) { /* (count) */
  Var r;
  package p;
...
static package bf_floatstr(Var arglist, Byte next, void *vdata, Objid progr) { /* (float, precision [, sci-notation]) */
  double d = arglist.v.list[1].v.fnum;
  int prec = arglist.v.list[2].v.num;
  int use_sci = (arglist.v.list[0].v.num >= 3 && is_true(arglist.v.list[3]));
...
static inline Var distance_euclidean(Var p, Var q) {
  auto sz = p.v.list[0].num();

  if (!p.is_list() || !q.is_list()) {
    return Var::new_error(E_TYPE);
  } else if (sz != q.v.list[0].num()) {
    return Var::new_error(E_INVARG);
  }

  double ret = 0.0;
  for (auto i = 1; i <= sz; i++) ret += pow(q.v.list[i].fnum() - p.v.list[i].fnum(), 2);

  return Var::new_float(sqrt(ret));
}

// Calculates the euclidean distance between n-dimensional coordinates.
static package bf_distance(Var arglist, Byte next, void *vdata, Objid progr) {
  Var ret;

  auto nargs = arglist.v.list[0].num();

  if (nargs <= 1) {
    free_var(arglist);
    return make_error_pack(E_INVARG);
  } else if (nargs == 2) {
    ret = distance_euclidean(arglist.v.list[1], arglist.v.list[2]);
  } else {
    ret = new_list(nargs - 1);

    for (auto i = 2; i <= nargs; i++) ret.v.list[i - 1] = distance_euclidean(arglist.v.list[1], arglist.v.list[i]);
  }

  free_var(arglist);
  return ret.is_error() ? make_error_pack(ret.v.err) : make_var_pack(ret);
}

/* Calculates the bearing between two sets of three dimensional floating point coordinates. */
static package bf_relative_heading(Var arglist, Byte next, void *vdata, Objid progr) {
  if (
    arglist.v.list[1].v.list[1].type != TYPE_FLOAT || arglist.v.list[1].v.list[2].type != TYPE_FLOAT || arglist.v.list[1].v.list[3].type != TYPE_FLOAT ||
    arglist.v.list[2].v.list[1].type != TYPE_FLOAT || arglist.v.list[2].v.list[2].type != TYPE_FLOAT || arglist.v.list[2].v.list[3].type != TYPE_FLOAT) {
    free_var(arglist);
    return make_error_pack(E_TYPE);
  }
...
static package bf_lerp(Var arglist, Byte next, void *vdata, Objid progr) {
  ...
  for (auto i = 1; i <= sz; i++) ret.v.list[i].fnum(start[i - 1].fnum() + ((end[i - 1].fnum() - start[i - 1].fnum()) * f));
...
void register_numbers(void) {
  ...
  register_function("distance",         1, -1, bf_distance, TYPE_LIST, TYPE_LIST);
